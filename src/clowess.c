...
/*  Comment from Lizhong Chen:
 *
 *  The file is copied straightly from 'lowess.c' in the 'stats' package of 
 *  R's core installation (R 4.4.0), with a few modifications:
 *
 *  - commented out all unnecessary headfiles.
 *  - commented out the error call.
 *  - remove RBoolean, using int instead. 
 * 	- remove debug check.
 * 	- remove R-C interface, SEXP .Call function.
...
#include <R_ext/Utils.h>	/* rPsort() and sort.c */
...
void lowest(double *x, double *y, int n, double *xs, double *ys, int nleft, int nright, double *w, int iter, double *rw, int *notok)
{
    int nrt, j;
...
{
    int i, iter, j, last, m1, m2, nleft, nright, ns, notok;
	...
	for(;;) {
	    ...
	    lowest(&x[1], &y[1], n, &x[i], &ys[i], nleft, nright, res, iter, rw, &notok);
	    if (notok) ys[i] = y[i];
