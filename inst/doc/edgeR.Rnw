...
By default, \texttt{calcNormFactors} uses the first sample as the reference, but this can be specified by the \texttt{refColumn} argument.  Also, you can specify different levels of trimming on the log-ratios or log-concentrations, as well as a cutoff on the log-concentrations.   From the calculated factors, you'll notice that since the reference is a kidney sample, all of the other kidney (technical) replicates are very near to 1, whereas all of the liver factors are less than 1, reflective of the bias.  Consider a smear plot between the first and second sample, with the normalization factor overlaid and then a similar plot after normalization (see Figure \ref{fig:Marioni_normalization}):
